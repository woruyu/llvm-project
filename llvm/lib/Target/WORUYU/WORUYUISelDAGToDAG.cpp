//===-- WORUYUISelDAGToDAG.cpp - A dag to dag inst selector for WORUYU ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines a DAG pattern matching instruction selector for WORUYU,
// converting from a legalized dag to a WORUYU dag.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "WORUYU-isel"
#define PASS_NAME "WORUYU DAG->DAG Pattern Instruction Selection"
...
class WORUYUDAGToDAGISel : public SelectionDAGISel {
  ...
  /// Subtarget - Keep a pointer to the WORUYUSubtarget around so that we can
  /// make the right decision when generating code for different subtargets.
  const WORUYUSubtarget *Subtarget;

public:
  WORUYUDAGToDAGISel() = delete;

  explicit WORUYUDAGToDAGISel(WORUYUTargetMachine &TM)
      : SelectionDAGISel(TM), Subtarget(nullptr) {}

  bool runOnMachineFunction(MachineFunction &MF) override {
    // Reset the subtarget each time through.
    Subtarget = &MF.getSubtarget<WORUYUSubtarget>();
...
class WORUYUDAGToDAGISelLegacy : public SelectionDAGISelLegacy {
public:
  static char ID;
  WORUYUDAGToDAGISelLegacy(WORUYUTargetMachine &TM)
      : SelectionDAGISelLegacy(ID, std::make_unique<WORUYUDAGToDAGISel>(TM)) {}
};
} // namespace

char WORUYUDAGToDAGISelLegacy::ID = 0;

INITIALIZE_PASS(WORUYUDAGToDAGISelLegacy, DEBUG_TYPE, PASS_NAME, false, false)

// ComplexPattern used on WORUYU Load/Store instructions
bool WORUYUDAGToDAGISel::SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset) {
...
// ComplexPattern used on WORUYU FI instruction
bool WORUYUDAGToDAGISel::SelectFIAddr(SDValue Addr, SDValue &Base,
                                   SDValue &Offset) {
  SDLoc DL(Addr);
...
bool WORUYUDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void WORUYUDAGToDAGISel::Select(SDNode *Node) {
...
void WORUYUDAGToDAGISel::PreprocessLoad(SDNode *Node,
...
void WORUYUDAGToDAGISel::PreprocessISelDAG() {
...
bool WORUYUDAGToDAGISel::getConstantFieldValue(const GlobalAddressSDNode *Node,
...
bool WORUYUDAGToDAGISel::fillGenericConstant(const DataLayout &DL,
                                          const Constant *CV,
                                          val_vec_type &Vals, uint64_t Offset) {
...
bool WORUYUDAGToDAGISel::fillConstantDataArray(const DataLayout &DL,
...
bool WORUYUDAGToDAGISel::fillConstantArray(const DataLayout &DL,
...
bool WORUYUDAGToDAGISel::fillConstantStruct(const DataLayout &DL,
...
void WORUYUDAGToDAGISel::PreprocessTrunc(SDNode *Node,
                                      SelectionDAG::allnodes_iterator &I) {
  ...
  // when the Reg operand comes from WORUYU_load_[byte | half | word] for
...
FunctionPass *llvm::createWORUYUISelDag(WORUYUTargetMachine &TM) {
  return new WORUYUDAGToDAGISelLegacy(TM);
}
