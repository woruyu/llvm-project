//===-- WORUYUISelLowering.cpp - WORUYU DAG Lowering Implementation  ------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that WORUYU uses to lower LLVM code into a
// selection DAG.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "WORUYU-lower"

static cl::opt<bool> WORUYUExpandMemcpyInOrder("WORUYU-expand-memcpy-in-order",
  cl::Hidden, cl::init(false),
  cl::desc("Expand memcpy into load/store pairs in order"));

static cl::opt<unsigned> WORUYUMinimumJumpTableEntries(
    "WORUYU-min-jump-table-entries", cl::init(13), cl::Hidden,
    cl::desc("Set minimum number of entries to use a jump table on WORUYU"));
...
WORUYUTargetLowering::WORUYUTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i64, &WORUYU::GPRRegClass);
  if (STI.getHasAlu32())
    addRegisterClass(MVT::i32, &WORUYU::GPR32RegClass);

  // Compute derived properties from the register classes
  computeRegisterProperties(STI.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(WORUYU::R11);
  ...
  setMinimumJumpTableEntries(WORUYUMinimumJumpTableEntries);

  // Function alignments
  setMinFunctionAlignment(Align(8));
  setPrefFunctionAlignment(Align(8));

  if (WORUYUExpandMemcpyInOrder) {
    ...
    // will cause trouble to memcpy pattern matcher inside kernel eWORUYU JIT
    // compilers.
    //
    // When -WORUYU-expand-memcpy-in-order specified, we want to defer the expand
...
bool WORUYUTargetLowering::allowsMisalignedMemoryAccesses(EVT VT, unsigned, Align,
...
bool WORUYUTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  return false;
}

bool WORUYUTargetLowering::isTruncateFree(Type *Ty1, Type *Ty2) const {
...
bool WORUYUTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool WORUYUTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
...
bool WORUYUTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
...
bool WORUYUTargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
unsigned WORUYUTargetLowering::getJumpTableEncoding() const {
  return MachineJumpTableInfo::EK_BlockAddress;
}

WORUYUTargetLowering::ConstraintType
WORUYUTargetLowering::getConstraintType(StringRef Constraint) const {
...
WORUYUTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &WORUYU::GPRRegClass);
    case 'w':
      if (HasAlu32)
        return std::make_pair(0U, &WORUYU::GPR32RegClass);
      break;
    default:
      break;
    }
...
void WORUYUTargetLowering::ReplaceNodeResults(
...
SDValue WORUYUTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
#include "WORUYUGenCallingConv.inc"
...
SDValue WORUYUTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, getHasAlu32() ? CC_WORUYU32 : CC_WORUYU64);
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i64:
        Register VReg = RegInfo.createVirtualRegister(
            SimpleTy == MVT::i64 ? &WORUYU::GPRRegClass : &WORUYU::GPR32RegClass);
...
const size_t WORUYUTargetLowering::MaxArgs = 5;
...
SDValue WORUYUTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // WORUYU target does not support tail call optimization.
  IsTailCall = false;
  ...
  CCInfo.AnalyzeCallOperands(Outs, getHasAlu32() ? CC_WORUYU32 : CC_WORUYU64);
  ...
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    if (StringRef(E->getSymbol()) != WORUYU_TRAP) {
  ...
  bool HasFastCall =
      (CLI.CB && isa<CallInst>(CLI.CB) && CLI.CB->hasFnAttr("WORUYU_fastcall"));
  ...
  if (HasFastCall) {
    ...
    if (!CLI.CB->getType()->isVoidTy())
      resetRegMaskBit(TRI, RegMask, WORUYU::R0);
  ...
  Chain = DAG.getNode(WORUYUISD::CALL, CLI.DL, NodeTys, Ops);
  InGlue = Chain.getValue(1);
...
SDValue
WORUYUTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
                               const SDLoc &DL, SelectionDAG &DAG) const {
  unsigned Opc = WORUYUISD::RET_GLUE;
  ...
  CCInfo.AnalyzeReturn(Outs, getHasAlu32() ? RetCC_WORUYU32 : RetCC_WORUYU64);
...
SDValue WORUYUTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, getHasAlu32() ? RetCC_WORUYU32 : RetCC_WORUYU64);
...
SDValue WORUYUTargetLowering::LowerSDIVSREM(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  fail(DL, DAG,
...
SDValue WORUYUTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
...
SDValue WORUYUTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(WORUYUISD::BR_CC, DL, Op.getValueType(), Chain, LHS, RHS,
                     DAG.getConstant(CC, DL, LHS.getValueType()), Dest);
}

SDValue WORUYUTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(WORUYUISD::SELECT_CC, DL, Op.getValueType(), Ops);
}

SDValue WORUYUTargetLowering::LowerATOMIC_LOAD_STORE(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDNode *N = Op.getNode();
  SDLoc DL(N);
...
static Function *createWORUYUUnreachable(Module *M) {
  if (auto *Fn = M->getFunction(WORUYU_TRAP))
    return Fn;

  FunctionType *FT = FunctionType::get(Type::getVoidTy(M->getContext()), false);
  Function *NewF =
      Function::Create(FT, GlobalValue::ExternalWeakLinkage, WORUYU_TRAP, M);
  ...
  DISubprogram *SP =
      DBuilder.createFunction(CU, WORUYU_TRAP, WORUYU_TRAP, nullptr, 0, FuncType, 0,
                              DINode::FlagZero, DISubprogram::SPFlagZero);
  NewF->setSubprogram(SP);
  return NewF;
}

SDValue WORUYUTargetLowering::LowerTRAP(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  ...
  Function *Fn = createWORUYUUnreachable(MF.getFunction().getParent());
  auto PtrVT = getPointerTy(MF.getDataLayout());
...
SDValue WORUYUTargetLowering::LowerJumpTable(SDValue Op, SelectionDAG &DAG) const {
...
SDValue WORUYUTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
                                   unsigned Flags) const {
  SDLoc DL(N);

  SDValue GA = getTargetNode(N, DL, MVT::i64, DAG, Flags);

  return DAG.getNode(WORUYUISD::Wrapper, DL, MVT::i64, GA);
}

SDValue WORUYUTargetLowering::LowerGlobalAddress(SDValue Op,
                                              SelectionDAG &DAG) const {
  ...
  return SDValue(DAG.getMachineNode(WORUYU::LDIMM64, DL, MVT::i64, Addr), 0);
}

SDValue WORUYUTargetLowering::LowerConstantPool(SDValue Op,
                                             SelectionDAG &DAG) const {
...
SDValue WORUYUTargetLowering::LowerBlockAddress(SDValue Op,
                                             SelectionDAG &DAG) const {
...
unsigned
WORUYUTargetLowering::EmitSubregExt(MachineInstr &MI, MachineBasicBlock *BB,
                                 unsigned Reg, bool isSigned) const {
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  const TargetRegisterClass *RC = getRegClassFor(MVT::i64);
  int RShiftOp = isSigned ? WORUYU::SRA_ri : WORUYU::SRL_ri;
  ...
  if (!isSigned) {
    ...
    BuildMI(BB, DL, TII.get(WORUYU::MOV_32_64), PromotedReg0).addReg(Reg);
    return PromotedReg0;
  }
  Register PromotedReg0 = RegInfo.createVirtualRegister(RC);
  ...
  if (HasMovsx) {
    BuildMI(BB, DL, TII.get(WORUYU::MOVSX_rr_32), PromotedReg0).addReg(Reg);
  } else {
    BuildMI(BB, DL, TII.get(WORUYU::MOV_32_64), PromotedReg0).addReg(Reg);
    BuildMI(BB, DL, TII.get(WORUYU::SLL_ri), PromotedReg1)
...
MachineBasicBlock *
WORUYUTargetLowering::EmitInstrWithCustomInserterMemcpy(MachineInstr &MI,
  ...
  // This function does custom insertion during lowering WORUYUISD::MEMCPY which
  // only has two register operands from memcpy semantics, the copy source
  // address and the copy destination address.
  //
  // Because we will expand WORUYUISD::MEMCPY into load/store pairs, we will need
  ...
  ScratchReg = MRI.createVirtualRegister(&WORUYU::GPRRegClass);
...
MachineBasicBlock *WORUYUTargetLowering::EmitInstrWithCustomInserterLDimm64(
    MachineInstr &MI, MachineBasicBlock *BB) const {
  MachineFunction *MF = BB->getParent();
  const WORUYUInstrInfo *TII = MF->getSubtarget<WORUYUSubtarget>().getInstrInfo();
  const TargetRegisterClass *RC = getRegClassFor(MVT::i64);
  MachineRegisterInfo &RegInfo = MF->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  if (MO.isBlockAddress()) {
    ...
    BuildMI(*BB, MI, DL, TII->get(WORUYU::LD_imm64), TmpReg)
        .addJumpTableIndex(JTI);
    BuildMI(*BB, MI, DL, TII->get(WORUYU::LDD), ResultReg)
        .addReg(TmpReg)
        .addImm(0);
    MI.eraseFromParent();
    return BB;
  ...
  auto emitLDImm64 = [&](const GlobalValue *GV = nullptr, unsigned JTI = -1) {
    auto MIB = BuildMI(*BB, MI, DL, TII->get(WORUYU::LD_imm64), ResultReg);
    if (GV)
...
MachineBasicBlock *
WORUYUTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                               MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();
  unsigned Opc = MI.getOpcode();
  bool isSelectRROp = (Opc == WORUYU::Select ||
                       Opc == WORUYU::Select_64_32 ||
                       Opc == WORUYU::Select_32 ||
                       Opc == WORUYU::Select_32_64);

  bool isMemcpyOp = Opc == WORUYU::MEMCPY;
  bool isLDimm64Op = Opc == WORUYU::LDIMM64;

#ifndef NDEBUG
  bool isSelectRIOp = (Opc == WORUYU::Select_Ri ||
                       Opc == WORUYU::Select_Ri_64_32 ||
                       Opc == WORUYU::Select_Ri_32 ||
                       Opc == WORUYU::Select_Ri_32_64);
  ...
  bool is32BitCmp = (Opc == WORUYU::Select_32 ||
                     Opc == WORUYU::Select_32_64 ||
                     Opc == WORUYU::Select_Ri_32 ||
                     Opc == WORUYU::Select_Ri_32_64);
...
#define SET_NEWCC(X, Y) \
  case ISD::X: \
    if (is32BitCmp && HasJmp32) \
      NewCC = isSelectRROp ? WORUYU::Y##_rr_32 : WORUYU::Y##_ri_32; \
    else \
      NewCC = isSelectRROp ? WORUYU::Y##_rr : WORUYU::Y##_ri; \
    break
  ...
  // eWORUYU at the moment only has 64-bit comparison. Any 32-bit comparison need
  ...
  // try to remove those unnecessary in WORUYUMIPeephole pass.
  if (is32BitCmp && !HasJmp32)
  ...
  // ...
  BB = Copy1MBB;
  BuildMI(*BB, BB->begin(), DL, TII.get(WORUYU::PHI), MI.getOperand(0).getReg())
...
EVT WORUYUTargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                          EVT VT) const {
  return getHasAlu32() ? MVT::i32 : MVT::i64;
}

MVT WORUYUTargetLowering::getScalarShiftAmountTy(const DataLayout &DL,
                                              EVT VT) const {
  return (getHasAlu32() && VT == MVT::i32) ? MVT::i32 : MVT::i64;
}

bool WORUYUTargetLowering::isLegalAddressingMode(const DataLayout &DL,
